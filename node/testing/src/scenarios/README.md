...
2. Whether the OCaml node can discover and connect to an Openmina node that is
   ...
   OCaml node can discover it and connect to the node.
    ...
    node to connect to it explicitly, it should find it automatically and
    connect using peer discovery (performed through Kademlia). This test will
    ensure the Openmina node uses Kademlia in a way that is compatible with the
    OCaml node.
...
* The malicious peer can deliberately choose the peer_id and deny access to the
  information, just always say there are no providers.
This problem has been inherited from the OCaml implementation of the node. We
have mitigated it by making the Openmina node not rely on GetProviders, instead,
we only do AddProviders to advertise ourselves, but treat any peer of the
Kademlia network as a valid Mina peer, no matter if it is a provider or not, so
a malicious peer can prevent OCaml nodes from discovering us, but it will not
...
* Kademlia exposes both internal and external addresses, which may be
  unnecessary in the MINA network and could be a security risk.
    ...
    If we test on any local network that belongs to these ranges because they
    will get filtered unless we manually disable (in code) these checks.
