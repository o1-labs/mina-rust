...
export const leaderboardReducer = createReducer(
  initialState,
  on(
    LeaderboardActions.getHeartbeatsSuccess,
    (state, { heartbeatSummaries }) => ({
      ...state,
      isLoading: false,
      heartbeatSummaries,
      filteredHeartbeatSummaries: sortHeartbeats(
        filterHeartbeats(heartbeatSummaries, state.filters),
        state.sortBy,
      ),
    }),
  ),
  on(LeaderboardActions.changeFilters, (state, { filters }) => ({
    ...
    filters,
    filteredHeartbeatSummaries: sortHeartbeats(
      filterHeartbeats(state.heartbeatSummaries, filters),
      state.sortBy,
    ),
  })),
  on(LeaderboardActions.sort, (state, { sort }) => ({
    ...
    sortBy: sort,
    filteredHeartbeatSummaries: sortHeartbeats(
      state.filteredHeartbeatSummaries,
      sort,
    ),
  })),
);

function sortHeartbeats(
  node: HeartbeatSummary[],
  tableSort: TableSort<HeartbeatSummary>,
): HeartbeatSummary[] {
